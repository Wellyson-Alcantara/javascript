...
function calc() {
    ...
    let n2 = Number(prompt('Segundo valo:'))
    ...
    switch (op) { // A estrutura switch basicamente  permite você testar valores dentro de uma variável ou expressão. Ela é compatível com números inteiros e strings.
        case 1:
            saida.innerHTML += `<p>${n1} + ${n2} = <strong>${n1 + n2}</strong></p>`
            break // O break é obrigatório em cada case. Se não for colocado, acontecerá a execução de vários comandos indesejados de outos cases.
        case 2:
            saida.innerHTML += `<p>${n1} - ${n2} = <strong>${n1 + n2}</strong></p>`
            break
        case 3:
